...
namespace Microsoft.AspNet.Routing.Template
{
    public static class TemplateParser
    {
        ...
        private static bool ParseParameter(TemplateParserContext context, TemplateSegment segment)
        {
            ...
            var templatePart = InlineRouteTemplateParser.ParseRouteParameter(rawParameter,
            ...
            if (templatePart.IsOptional && templatePart.DefaultValue != null)
            {
                ...
                // This used to be treated as a default value including the '?' however today the only way to declare if 
                // a template parameter is optional is to have a ? at the end, 
                // hence we cannot have both default value and optional parameter within the template. 
                // We should consider a way for the scenarios where user wants to provide a default value which ends with a ?.
                // Today the only way to do that would be to add it as a separate entry in the dictionary.
